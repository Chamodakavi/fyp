...
export const cropManager = {
    ...
    try {
      ...
      if (!targetData) {
        return {
          ...
          remaining: 0,
          isFull: true,
          error: "Crop not found in national targets",
        };
      }
      ...
      const currentFilled = filledData.reduce(
        (sum, row) => sum + row.amount_mt,
        0,
      );
  ...
  async registerCrop(farmerId: string, cropName: string, amountMt: number) {
    ...
    if (!status)
      return {
        status: "error",
        message: "System Error: Could not fetch crop limits.",
      };

    if (status.error) {
      return {
        status: "rejected",
        message: `Crop '${cropName}' is not registered in the national targets.`,
      };
    }
    ...
    if (amountMt > status.remaining) {
      return {
        status: "rejected",
        message: `Quota Exceeded! Only ${status.remaining} MT left.`,
      };
    }
    ...
    const { error } = await supabase
      .from("farmer_registrations")
      .insert([
        { farmer_id: farmerId, crop_name: cropName, amount_mt: amountMt },
      ]);
